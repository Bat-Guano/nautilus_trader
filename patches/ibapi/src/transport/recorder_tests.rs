...
/// Local stand-in for the `temp-env` dev-dependency, which is unavailable
/// in the offline registry cache this workspace builds against. Mirrors
/// `temp_env::with_var(key, value, closure)`: sets the variable for the
/// duration of the closure and restores the previous value afterwards.
fn with_recording_dir<T>(key: &str, value: Option<&str>, f: impl FnOnce() -> T) -> T {
...
fn test_message_recorder_new_with_empty_env_var() {
    with_recording_dir("IBAPI_RECORDING_DIR", Some(""), || {
...
fn test_message_recorder_new_with_valid_env_var() {
    ...
    with_recording_dir("IBAPI_RECORDING_DIR", Some(temp_path), || {
        let recorder = MessageRecorder::from_env();
