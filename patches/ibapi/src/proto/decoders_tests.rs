...
#[test]
fn parse_required_unknown_preserves_identity() {
    let v: OrderStatusKind = parse_required(Some("Garbage"), "OrderStatus").unwrap();
    assert_eq!(v, OrderStatusKind::Unknown("Garbage".into()));
}
...
#[test]
fn parse_optional_unknown_preserves_identity() {
    let v: Option<OrderStatusKind> = parse_optional(Some("Garbage")).unwrap();
    assert_eq!(v, Some(OrderStatusKind::Unknown("Garbage".into())));
}
