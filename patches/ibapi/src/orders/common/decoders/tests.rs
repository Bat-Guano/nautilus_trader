...
// === C2.6: unknown order-status vocabulary decodes with identity intact ===

#[test]
fn test_decode_order_status_unknown_status_preserved() {
    use prost::Message;

    let proto_msg = crate::proto::OrderStatus {
        order_id: Some(99),
        status: Some("SomeFutureIbkrStatus".into()),
        filled: Some("50".into()),
    ...
    assert_eq!(result.status, OrderStatusKind::Unknown("SomeFutureIbkrStatus".into()));
    assert_eq!(result.status.as_str(), "SomeFutureIbkrStatus");
}

#[test]
fn test_decode_order_status_missing_status_still_errors() {
    use prost::Message;
...
#[test]
fn test_order_update_stream_decoder_unknown_status_is_not_terminal() {
    ...
    let mut message = ResponseMessage::from_protobuf(IncomingMessages::OrderStatus as i32, bytes);
    let context = DecoderContext::default();

    let decoded = OrderUpdate::decode(&context, &mut message)
        .expect("unknown status vocabulary must not produce a decode error");

    match decoded {
        OrderUpdate::OrderStatus(status) => {
            assert_eq!(status.order_id, 5);
            assert_eq!(status.perm_id, 7);
            assert_eq!(status.client_id, 3);
            assert_eq!(status.status, OrderStatusKind::Unknown("SomeFutureIbkrStatus".into()));
        }
        other => panic!("expected OrderStatus update, got {other:?}"),
    }
}
