...
pub enum OrderStatusKind {
    ...
    /// cancelled. Terminal.
    Inactive,
    /// A status string outside the known TWS vocabulary, preserved exactly as
    /// received on the wire (C2.6). Unknown statuses decode successfully so a
    /// future IBKR status cannot terminate the order-update stream; the raw
    /// value remains available through [`as_str`](Self::as_str).
    ///
    /// Unknown is deliberately neither [`active`](Self::is_active) nor
...
impl OrderStatusKind {
    ...
    ///
    /// Known variants return their exact wire strings;
    /// [`Unknown`](Self::Unknown) returns the stored original string
    /// byte-for-byte.
    ...
    /// [`is_terminal`](Self::is_terminal) together cover 8 of 10 variants —
    /// `ApiPending` and [`Unknown`](Self::Unknown) are neither, so do not
    /// assume `!is_active() ⇒ is_terminal()`.
...
impl std::str::FromStr for OrderStatusKind {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        match Self::from_wire(s) {
            Some(kind) => Ok(kind),
            // Unknown non-empty status strings decode successfully with their
            // identity intact (C2.6): a future IBKR vocabulary entry must not
            // terminate the order-update stream. An empty string remains a
            // parse error - a missing required wire field is a protocol bug.
            None if !s.is_empty() => Ok(Self::Unknown(s.to_string())),
            None => Err(crate::Error::Parse(0, s.to_string(), "unknown OrderStatusKind".into())),
        }
    }
}

impl crate::ToField for OrderStatusKind {
    fn to_field(&self) -> String {
        self.to_string()
    }
}
